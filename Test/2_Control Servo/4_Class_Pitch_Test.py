...
import cv2
import time
from class_Pitch import Pitch  # Importa la clase Pitch
...
# Mostrar ventana antes de comenzar el bucle principal
frame_inicial = np.zeros((FRAME_HEIGHT, FRAME_WIDTH, 3), dtype=np.uint8)
cv2.putText(frame_inicial, "Iniciando...", (200, 240), font, 1, (255, 255, 255), 2)
cv2.imshow("Pitch Tracking Test", frame_inicial)
cv2.waitKey(500)
...
while True:
    ...
    cv2.imshow("Pitch Tracking Test", gray_bgr)
    ...
    if key == ord('q'):
        ...
        try:
            ...
            print(f"Corrigiendo pitch con error Y = {pix}")
            pitch_servo.mover_por_pixeles(pix)
            time.sleep(0.5)
        except:
