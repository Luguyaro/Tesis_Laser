...
def mover_a_pasos(pasos_objetivo, sentido='1'):
    ...
    try:
        while step_count < pasos_objetivo:
            ...
            pwm = 0.1 if restante < 20 else 0.3
            if sentido == '1':
...
def detectar_objeto_y_mover():
    ...
    while True:
        ...
        mostrar_frame = cv2.rotate(frame.copy(), cv2.ROTATE_180)
        gray = cv2.rotate(gray.copy(), cv2.ROTATE_90_CLOCKWISE)
        ...
        cv2.imshow("Tracking Cam", mostrar_frame)
